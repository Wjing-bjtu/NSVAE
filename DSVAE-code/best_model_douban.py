...
class dual_ssl_loss (nn.Module): 
    ...
    def forward(self, d_q, d_k):
        ...
        for l in range(2):
           ...
           # print('q',q,q.shape)
            k = d_k[l]
            queue = torch.randn(self.dim[l], self.sample_num).cuda()#128是负样本个数  
...
class VAE(nn.Module):
    def __init__(self, hidden_dim, latent_dim, input_dim, ssl_temp, ssl_reg, cont_temp, sample_num, con_reg):
    ...
    def forward(self, user_ratings, sub1, sub2, n_users, beta=None, gamma=1, dropout_rate=0.5, calculate_loss=True):
        ...
        if calculate_loss:
            ...
            negative_elbo = -(mll - 1.*kld) + ssl + self.con_reg * dual_loss#0.05  
