...
class dual_ssl_loss (nn.Module): 
    def __init__(self, cont_temp, hidden_dim, sample_num):
    ...
    def forward(self, d_q, d_k):
        ...
        for l in range(2):
           ...
           # print('q',q,q.shape)
            k = d_k[l]
            queue = torch.randn(self.dim[l], self.sample_num).cuda()#80是负样本个数  
...
class VAE(nn.Module):
    ...
    def forward(self, user_ratings, sub1, sub2, n_users, beta=None, gamma=1, dropout_rate=0.5, calculate_loss=True):
        ...
        if calculate_loss:
            ...
            negative_elbo = -(mll - 1.*kld) + ssl + self.cont_reg * dual_loss#0.05  0.2
...
class VAE(nn.Module):
    ...
    def forward(self, user_ratings, sub1, sub2, n_users, beta=None, gamma=1, dropout_rate=0.5, calculate_loss=True):
        ...
        if calculate_loss:
            ...
            ssl = self.ssl_loss(z_sub1, z_sub2, n_users)
            dual_loss = self.dual_loss([h5, h6], [h2, h3])
            negative_elbo = -(mll - 1.*kld) + ssl + dual_loss#0.05  0.2
