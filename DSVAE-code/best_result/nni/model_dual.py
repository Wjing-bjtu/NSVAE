...
class dual_ssl_loss (nn.Module): 
    def __init__(self, cont_temp, hidden_dim, cont_reg, sample):
    ...
    def forward(self, d_q, d_k):
        ...
        for l in range(2):
            ...
            #print('d_q[l]',d_q[l].shape)
            #print('q',q.shape)
            k = d_k[l]
            #print('k',k.shape)
            queue = torch.randn(self.dim[l], self.sample).cuda()#128 negative sample
            ...
            cont_loss += cont_loss
        return self.cont_reg * cont_loss
...
class VAE(nn.Module):
    def __init__(self, hidden_dim, latent_dim, input_dim, ssl_temp, ssl_reg, cont_temp, cont_reg, sample_num):
        ...
        self.dual_loss = dual_ssl_loss(cont_temp, [hidden_dim, input_dim], cont_reg, sample_num)
    ...
    def forward(self, user_ratings, sub1, sub2, n_users, beta=None, gamma=1, dropout_rate=0.5, calculate_loss=True):
        ...
        if calculate_loss:
            ...
            negative_elbo = -(mll - 1.*kld) + ssl + 0.2 * dual_loss#0.05  0.2
...
class dual_ssl_loss (nn.Module): 
    def __init__(self, cont_temp, hidden_dim, sample):
        super(dual_ssl_loss, self).__init__()
        self.cont_temp = cont_temp
        self.dim = hidden_dim
        self.sample = sample
    ...
    def forward(self, d_q, d_k):
        ...
        for l in range(2):
           ...
           # print('q',q,q.shape)
            k = d_k[l]
            queue = torch.randn(self.dim[l], self.sample).cuda()#128 negative sample  256
...
class VAE(nn.Module):
    def __init__(self, hidden_dim, latent_dim, input_dim, ssl_temp, ssl_reg, cont_temp, sample_num):
        super(VAE, self).__init__()
        ...
        self.ssl_loss = calc_ssl_loss(ssl_temp, ssl_reg)
        self.dualencoder = Dual_Encoder(hidden_dim, input_dim)
        self.dual_loss = dual_ssl_loss(cont_temp, [hidden_dim, input_dim], sample_num)
    ...
    def forward(self, user_ratings, sub1, sub2, n_users, beta=None, gamma=1, dropout_rate=0.5, calculate_loss=True):
        ...
        if calculate_loss:
            ...
            ssl = self.ssl_loss(z_sub1, z_sub2, n_users)
            dual_loss = self.dual_loss([h5, h6], [h2, h3])
            negative_elbo = -(mll - 1.*kld) + ssl + 0.05 * dual_loss#0.05  0.2
