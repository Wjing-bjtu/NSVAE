...
from best_model_epinion import VAE
#os.environ["CUDA_VISIBLE_DEVICES"] = "0"  
import argparse
parser = argparse.ArgumentParser()
parser.add_argument('--dataset', default='./ml-10m',type=str)
...
parser.add_argument('--lr', type=float, default=5e-4)#  5e-4   1e-3
...
train_mask1, train_mask2 = mask_percent(data_all, data_sub1,data_sunb2, 0.1)#0.1
...
print('train_data.shape[1]',train_data.shape[0])
model_kwargs = {
    'hidden_dim': args.hidden_dim,
    'latent_dim': args.latent_dim,
    'input_dim': train_data.shape[1],
    'ssl_reg': 0.01,#0.01
    'ssl_temp': 0.7,#0.5   0.6  0.7
    'cont_temp': 0.2#0.2
}
...
for epoch in range(args.n_epochs):
    ...
    print('valid_scores[-1]',valid_scores[-1])    
...
class VAE(nn.Module):
    ...
    def forward(self, user_ratings, sub1, sub2, n_users, beta=None, gamma=1, dropout_rate=0.5, calculate_loss=True):
        ...
        if calculate_loss:
            ...
            negative_elbo = -(mll - 1.*kld) + ssl + 0. * dual_loss#0.05  0.2
